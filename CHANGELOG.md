...
* `Start-TssSecretChangePassword`'s `-Type` parameter is no longer mandatory and defaults to `Random`. Scripts that previously relied on the mandatory-parameter binding error as a guard against forgetting `-Type` will now silently trigger a random rotation on affected secrets. Rotations are recoverable via Secret Server's password history if the change was unintentional. Fixes #444.
...
* `Initialize-TssSdkClient` - fix `Unrecognized command or argument 'X'` when `-RuleName` contains a space (e.g. `'Customer Onboarding'`). The cmdlet now passes arguments to `tss.exe` via `ProcessStartInfo.ArgumentList` (PS 7+ / .NET 5+ per-arg collection) instead of building a single space-joined `Arguments` string, so values with spaces, quotes, or other shell-special characters reach the SDK as a single token. The `-ConfigPath` parameter no longer rejects paths with whitespace (the previous `ValidateScript` was a workaround for the same root cause). Note: callers who previously used the workaround documented in #395 of pre-wrapping the rule name in literal quotes (e.g. `-RuleName '"Rule With Spaces"'`) must now drop the inner quotes; otherwise the literal quotes will be sent to the SDK as part of the rule name. Also fixes a `Write-Waring` typo in the Force-remove warning path and a copy-paste `Write-Verbose` message that reported "tss init" from the `tss remove` branch. Also masks the `--onboarding-key` value in the `Write-Verbose` output so `-Verbose` transcripts captured for support tickets do not leak the key. Fixes #395.
...
* `Update-TssMetadataField` - fix a copy-paste error in the `[OutputType()]` declaration (was `Thycotic.PowerShell.Folders.Folder`, now correctly `Thycotic.PowerShell.Metadata.Field`). No runtime behavior change - the cmdlet already returned the correct type via its cast; only Get-Help and IntelliSense were misinformed. Fixes part of #461.
