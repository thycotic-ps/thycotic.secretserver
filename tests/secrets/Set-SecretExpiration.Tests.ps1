...
Describe "$commandName verify parameters" {
    BeforeDiscovery {
        [object[]]$knownParameters = 'TssSession', 'Id', 'Interval', 'DateExpiration', 'Template'
...
Describe "$commandName functions" {
    Context "Checking" {
        BeforeAll {
            ...
            $secretId = 42
            Mock -Verifiable -CommandName Invoke-RestMethod -ParameterFilter { $Uri -match "/secrets/$secretId/expiration" } -MockWith {
                return [pscustomobject]@{
                    expirationType = 'Template'
                }
            }
            $object = Set-SecretExpiration -TssSession $session -Id $secretId -Interval 12
