...
Describe "$commandName verify parameters" {
    BeforeDiscovery {
        [object[]]$knownParameters = 'TssSession', 'Id', 'Comment', 'DoublelockPassword', 'TicketNumber', 'TicketSystemId', 'ForceCheckIn', 'IncludeInactive'
...
Describe "$commandName functions" {
    Context "Checking" {
        BeforeAll {
            ...
            $secretId = 42
            Mock -Verifiable -CommandName Invoke-RestMethod -ParameterFilter { $Uri -match "secrets/$secretId/expire" } -MockWith {
                return [pscustomobject]@{
                    id = $secretId
                }
            }
            $object = Revoke-Secret -TssSession $session -Id $secretId
