...
Describe "$commandName verify parameters" {
    BeforeDiscovery {
        [object[]]$knownParameters = 'TssSession', 'Id', 'CredentialOnSecret', 'PrivilegedSecretId'
        ...
        BeforeAll {
            ...
            Mock -Verifiable -CommandName Invoke-RestMethod -ParameterFilter { $Uri -match "/internals/secret-detail/$secretId/rpc"} -MockWith {
                return [pscustomobject]@{
                    privilegedAccountSecretId = [pscustomobject]@{
                        value = 111
                    }
                }
            }
            $object = Set-SecretRpcPrivileged -TssSession $session -Id $secretId -PrivilegedSecretId 111
