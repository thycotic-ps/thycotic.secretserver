        ...
        BeforeAll {
            ...
            Mock -Verifiable -CommandName Invoke-TssRestApi -ParameterFilter { $Uri -eq "$($session.ApiUrl)/secrets/$secretId/email"; $Method -eq 'PATCH' } -MockWith {
                return [pscustomobject]@{
                    ...
                    sendEmailWhenChanged        = @{fieldInputType = ""; sortOrder = ""; value = $false; label = 'Send Email When Changed'; name = ""; description = ""; readOnly = $false; placeholder = ""; hidden = $false; hideOnView = ""; hasHistory = ""; isRequired = ""; readOnlyReason = ""; helpLink = "" }
