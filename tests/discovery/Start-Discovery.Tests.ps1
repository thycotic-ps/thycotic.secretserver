...
Describe "$commandName functions" {
    Context "Checking" {
        BeforeAll {
            ...
            Mock -Verifiable -CommandName Invoke-RestMethod -ParameterFilter { $Uri -match '/discovery/run' } -MockWith {
                return $true
            }
            $object = Start-Discovery -TssSession $session -Type 'ComputerScan'
